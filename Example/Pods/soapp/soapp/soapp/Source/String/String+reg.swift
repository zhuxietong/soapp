...
public extension String{
    ...
    public func subStrings(withReg reg:String)->[String]
    {
        ...
        if let matches:[AnyObject] = internalExpression?.matches(in: self, options: NSRegularExpression.MatchingOptions.reportCompletion, range: NSMakeRange(0, self.len))
        {
...
//        let range = Range<String.Index>(start: index1, end: index2)
        ...
        if let modifyString = internalExpression?.stringByReplacingMatches(in: beginString, options: NSRegularExpression.MatchingOptions.reportCompletion, range: NSMakeRange(0, beginString.len), withTemplate: tag)
        {
...
//            let pattern = "[1-9][0-9]{4,14}"
    ...
    public func haveSubString(machReg reg:String) ->Bool
    {
        ...
        internalExpression?.enumerateMatches(in: tmp, options: NSRegularExpression.MatchingOptions.reportCompletion, range: NSMakeRange(0, tmp.len), using: { (result:NSTextCheckingResult?, flags:NSRegularExpression.MatchingFlags, pointer:UnsafeMutablePointer<ObjCBool>) -> Void in
