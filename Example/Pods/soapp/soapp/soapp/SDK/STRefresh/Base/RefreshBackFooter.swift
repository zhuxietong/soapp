...
//        let happenOffsetY = self.happenOffsetY
        ...
        let pullingPercent = (currentOffsetY - happenOffsetY) / co_height
    ...
    override func scrollViewContentSizeDidChange(change: NSDictionary?) {
        ...
        // 表格的高度
        let scrollHeight = scrollView.co_height - scrollViewOriginalInset.top - scrollViewOriginalInset.bottom + ignoredScrollViewContentInsetBottom
        // 设置位置和尺寸
        co_y = max(contentHeight, scrollHeight)
    }
        ...
        set {
            ...
            if newValue == .NoMoreData || newValue == .Idle {
                ...
                UIView.animate(withDuration: RefreshFastAnimationDuration,
                    animations: {
                        var bottom = self.co_height + self.scrollViewOriginalInset.bottom
                        ...
                        self.scrollView.offsetY = self.happenOffsetY + self.co_height
